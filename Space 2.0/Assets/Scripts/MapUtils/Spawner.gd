...
func _ready():
	for body in get_tree().get_nodes_in_group("PhysicalBody"):
		body.connect("changeType", self, "spawn")
...
func spawn(body, newType):
	var newPlanet = celestialObjects[newType].instance()
	...
	newPlanet.connect("changeType", self, "spawn")
	emit_signal("addBody", newPlanet)

func randomSpawn(pos: Vector2):
	var objects = ["Star", "Planet"]
	objects.shuffle()
	var newObj = celestialObjects[objects[0]].instance()
	...
	newObj.position = pos
	newObj.connect("changeType", self, "spawn")
